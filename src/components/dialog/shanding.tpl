<div id="shanding">
  <input type="hidden" value="" id="serviceTypeDefault">
  <input type="hidden" value="58460dbff8a8986c435634a0" id="orderGid">
  <input type="hidden" value="" id="orginNum">
  <div class="dialogbox open" style="margin-top: -178px;">
    <div class="dialog-title">预约看房
      <span class="summary"></span>
    </div>
    <em class="close"></em>
    <form id="yhtDialogRegForm">
      <input type="hidden" value="58460dbff8a8986c435634a0" id="dialoghiddenid" name="gid">
      <input type="hidden" name="houseId" value="58feaf85a093c04172cf8f4d">
      <input type="hidden" value="融创观澜壹号" name="residentialName">
      <input type="hidden" value="其他户型" name="houseTypeId">
      <input type="hidden" value="其他户型" name="houseTypeName">
      <ul class="dialogcontent">
        <li class="inputitems cf">
          <span class="inputitem f-l">
            <i class="notnull">*</i>
            <span class="inputname four">真实姓名</span>
            <span class="colon">:</span>
          </span>
          <span class="inputbox f-r">
            <input type="text" class="inputname" value="" id="joinGroupName" name="userName" placeholder="请输入您的真实姓名">
          </span>
        </li>
        <li class="inputitems cf">
          <span class="inputitem f-l">
            <i class="notnull">*</i>
            <span class="inputname four">联系电话</span>
            <span class="colon">:</span>
          </span>
          <span class="inputbox f-r">
            <input type="text" class="inputphone" id="joinGroupPhone" name="telephone" maxlength="11" value="" placeholder="请输入您的手机号码">
          </span>
        </li>
        <li class="inputitems cf">
            <span class="inputitem f-l">
              <i class="notnull">*</i>
              <span class="inputname four">图形验证</span>
              <span class="colon">:</span>
            </span>
                <span class="inputbox f-r cf">
              <input type="text" class="inputimgcode f-l" maxlength="4" placeholder="图形验证">
              <img class="img-check f-r" codeid="5a541ecfa093c02ab057181a" uid="31876A45A48B440D2C166E2C479A68B5DE03975AE4CE842C189A37A25219A296" src="data:image/png;base64," codeid="5a541ecfa093c02ab057181a" uid="31876A45A48B440D2C166E2C479A68B5DE03975AE4CE842C189A37A25219A296">
            </span>
            </li>
        <li class="remark">备注：联系电话仅亿房顾问联系您时使用,不做其他用途</li>
        <li class="inputitems cf" id="checkCodeFiled">
          <span class="inputitem f-l">
            <i class="notnull">*</i>
            <span class="inputname">验证码</span>
            <span class="colon">:</span>
          </span>
          <span class="inputbox f-r cf">
            <input type="text" class="inputcode" id="joinGroupCode" name="smsCode" maxlength="6" placeholder="请输入验证码">
            <span class="btn btn-default getcode f-r">获取验证码</span>
          </span>
        </li>
      </ul>
    </form>
    <div class="joinin" id="joinGroupSubmit" data-spm="fdc01.1005.pcxxx.a0005xxx">提交</div>
    <div class="loginbox" style="">
      您也可以
      <a href="http://uc.fdc.com.cn/login.html" class="login">登录</a>
      <span class="line">|</span>
      <a href="http://uc.fdc.com.cn/regmain.html" class="regist">注册</a>
    </div>
  </div>
  <div class="dialogmask"></div>
</div>